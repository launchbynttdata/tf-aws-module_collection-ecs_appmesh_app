# This example provisions the App Mesh app along with the supporting platform and ingress modules.

interface_vpc_endpoints = {
  ecrdkr = {
    service_name        = "ecr.dkr"
    private_dns_enabled = true
  }
  ecrapi = {
    service_name        = "ecr.api"
    private_dns_enabled = true
  }
  ecs = {
    service_name        = "ecs"
    private_dns_enabled = true
  }
  logs = {
    service_name        = "logs"
    private_dns_enabled = true
  }
  appmeshenvoymgmt = {
    service_name        = "appmesh-envoy-management"
    private_dns_enabled = true
  }
  appmesh = {
    service_name        = "appmesh"
    private_dns_enabled = true
  }
}

gateway_vpc_endpoints = {
  s3 = {
    service_name        = "s3"
    private_dns_enabled = true
  }
}

vpce_security_group = {
  ingress_rules       = ["https-443-tcp", "http-80-tcp"]
  ingress_cidr_blocks = ["10.1.0.0/16"]
  egress_rules        = ["all-all"]
  egress_cidr_blocks  = ["0.0.0.0/0"]
}

alb_sg = {
  description         = "Security group for ALB"
  ingress_cidr_blocks = ["10.1.0.0/16"]
  ingress_with_cidr_blocks = [
    {
      from_port = 443
      to_port   = 443
      protocol  = "tcp"
    },
    {
      from_port = 80
      to_port   = 80
      protocol  = "tcp"
    }
  ]
  egress_rules       = ["all-all"]
  egress_cidr_blocks = ["0.0.0.0/0"]
}

dns_zone_name = "sandbox.launch.nttdata.com"
private_zone  = false

private_ca_arn = "arn:aws:acm-pca:us-east-2:020127659860:certificate-authority/aae3aa9a-a2d0-42ac-a16b-0fc6d54b109c"

vgw_security_group = {
  egress_rules        = ["all-all"]
  egress_cidr_blocks  = ["0.0.0.0/0"]
  ingress_cidr_blocks = ["0.0.0.0/0"]
  ingress_with_cidr_blocks = [
    {
      from_port = 9901
      to_port   = 9901
      protocol  = "tcp"
    },
    {
      from_port = 443
      to_port   = 443
      protocol  = "tcp"
    }
  ]
}

ingress_app_image_tag = "public.ecr.aws/nginx/nginx:latest"

ingress_app_security_group = {
  egress_rules        = ["all-all"]
  egress_cidr_blocks  = ["0.0.0.0/0"]
  ingress_cidr_blocks = ["0.0.0.0/0"]
  ingress_with_cidr_blocks = [
    {
      from_port = 8080
      to_port   = 8080
      protocol  = "tcp"
    }
  ]
}

app_ports                          = [8080]
app_image_tag                      = "public.ecr.aws/nginx/nginx:latest"
app_health_check_path              = "/"
virtual_node_app_health_check_path = "/"

ecs_security_group = {
  ingress_cidr_blocks = ["10.1.0.0/16"]
  ingress_with_cidr_blocks = [
    {
      from_port = 8080
      to_port   = 8080
      protocol  = "tcp"
    }
  ]
  egress_rules       = ["all-all"]
  egress_cidr_blocks = ["0.0.0.0/0"]
}

match_path_prefix = "/app"

tags = {
  Purpose = "terratest examples"
  Env     = "sandbox"
}
